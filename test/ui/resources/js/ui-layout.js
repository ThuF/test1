	...
	this.init = function(containerEl, viewNames, id, reconstruct){
		...
		if(id){
			if(!reconstruct){
				...
				var savedState = localStorage.getItem('DIRIGIBLE.application.test.'+ id);
				if(savedState !== null) {
		...
		let focusView = localStorage.getItem("DIRIGIBLE.application.test.focusView");
		...
		if(id){
			this.layout.on('stateChanged', function(){
				...
				localStorage.setItem('DIRIGIBLE.application.test.'+ id, state );
			}.bind(this));			
		}
