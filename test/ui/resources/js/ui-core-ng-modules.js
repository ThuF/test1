...
.service('Perspectives', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/test/api/launchpad/perspectives.js');
}])
.service('Tiles', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/test/api/launchpad/tiles.js');
}])
.service('Menu', ['$resource', function($resource){
	return $resource(relativePath + 'services/v4/js/test/api/launchpad/menu.js');
}])
...
.factory('viewRegistry', ['ViewRegistrySvc', '$resource', 'ViewFactories', function(ViewRegistrySvc, $resource, ViewFactories){
	...
	var get = function(viewsExtensionPoint){
		return $resource(relativePath + 'services/v4/js/test/api/launchpad/views.js')
			.query({
...
.directive('menu', ['$resource', 'Theme', 'User', 'Layouts', 'Menu', 'messageHub', function($resource, Theme, User, Layouts, Menu, messageHub){
	return {
		...
		templateUrl: relativePath + 'services/v4/web/test/ui/resources/templates/menu.html'
	}
}])
.directive('sidebar', ['Perspectives', function(Perspectives){
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.activePerspective = localStorage.getItem('DIRIGIBLE.application.test.activePerspective');

			scope.setActivePerspective = function(activePerspective) {
				localStorage.setItem('DIRIGIBLE.application.test.activePerspective', activePerspective);
				scope.activePerspective = activePerspective;
			};
		},
		templateUrl: relativePath + 'services/v4/web/test/ui/resources/templates/sidebar.html'
	}
}])
	...
	return {
		...
		link: function(scope, el, attrs){
			...
			scope.setActiveView = function(activeView) {
				localStorage.setItem('DIRIGIBLE.application.test.activePerspective', activeView.group);
				localStorage.setItem('DIRIGIBLE.application.test.focusView', activeView.name);
			};
		},
		templateUrl: relativePath + 'services/v4/web/test/ui/resources/templates/tiles.html'
	}
}])
